    ...
    for pian in pagetree:
        for url in pian[1]:
            ...
            if lieshu == 4:
                ...
                i = 0
                def has_not_attrs(tag):
                    print(tag.name)
                    print(tag.attrs)
                    return((tag.name == 'td') and (tag.has_attr))
                tds = table.find_all(has_not_attrs)
                #a=[tds[i] for i in range(1,len(tds),10)]
                #b=[tds[i] for i in range(11,len(tds),10)]
                print(tds)
                for td in tds:
                    tm = td.string
                    pid = td.previous_sibling.string
...
def analyze_testpage(pid):
     html = S.get(BASE_URL + 'problem_show.php?pid=' + pid,headers=header).content
     html = str(html,encoding='utf-8',errors='ignore')
     soup = BeautifulSoup(html,'lxml')
